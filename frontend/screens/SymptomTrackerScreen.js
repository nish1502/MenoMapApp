...
const SymptomTrackerScreen = ({ navigation }) => {
  ...
  const onSaveAndPredict = async () => {
    ...
    try {
      ...
      if (response.ok && jsonResponse?.status === "success") {
        const resultData = jsonResponse.data;
        console.log("Stage Prediction:", resultData);
        setApiResult(resultData);
        setLatestLogId(resultData?.log_id);
        setModalVisible(true);
      } else {
        const errorMsg = jsonResponse?.message || jsonResponse?.error || 'Prediction failed.';
  ...
  const fetchRemedy = async () => {
    ...
    const remedyPayload = {
      log_id: latestLogId || apiResult?.log_id,
      user_id: userId,
    ...
    try {
      ...
      if (response.ok && json?.status === "success") {
        const remedyData = json.data;
        console.log("Remedy Response:", remedyData);
        setRemedyResult(remedyData);
        setLatestHistoryId(remedyData?.history_id);
        setRemedyModalVisible(true);
      } else {
        const errorMsg = json?.message || json?.error || 'Could not fetch remedy.';
        Alert.alert('Error', errorMsg);
      }
    } catch (e) {
            ...
            <View style={styles.modalBackdrop}>
              <View style={styles.modalView}>
                ...
                <Text style={styles.modalText}>
                  You may be entering{' '}
                  <Text style={{ fontWeight: 'bold' }}>{apiResult?.predicted_stage || "Unknown Stage"}</Text>.
                </Text>
          ...
          >
            <View style={styles.modalBackdrop}>
              <View style={styles.modalView}>
                ...
                <Text style={styles.modalText}>
                  For your {remedyResult?.target_symptom || "symptom"}, we recommend:
                </Text>
                <Text style={styles.remedyName}>{remedyResult?.best_remedy_name || "Suggested Care"}</Text>

                <View style={styles.instructionsBox}>
                  {(remedyResult?.instructions?.steps || []).length > 0 ? (
                    remedyResult?.instructions?.steps?.map((step, index) => (
                      <Text key={index} style={styles.instructionStep}>• {step}</Text>
                    ))
                  ) : (
                    <Text style={styles.instructionStep}>No specific instructions provided.</Text>
                  )}
                </View>
                <Text style={styles.modalSubText}>Initial Severity: {remedyResult?.initial_severity || "N/A"}</Text>
                <Text style={styles.modalSubText}>Predicted Outcome: {remedyResult?.predicted_outcome || "TBD"}</Text>
